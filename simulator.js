function runSim() {
  ...
  var curse = document.getElementByName("curse").value;
  var primary = document.getElementByName("primary").value;
  var finisher = document.getElementByName("finisher").value;
  if (curse == "debuff") {
    var threatTime = 1.5;
    var useAgony = false;
    var useDoom = false;}
  ...
  crit += 10*onyxiaBuff + 5*songflower + 3*diremaulBuff + 1*document.getElementById("brilliantOil").checked + 3*document.getElementById("moonkinAura").checked + 5*document.getElementById("talentDevastation").parentNode.children[1].innerHTML;
  ...
  for (var q=1; q<=6; q++) {
    ...
    var critSearing = (1.7 + crit + (intel/60.6) + 5*document.getElementById("talentSearingPain").parentNode.children[1].innerHTML) * (100-miss)/100;
